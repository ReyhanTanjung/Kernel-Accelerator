...
// Include the fractal header for parameter structure
#include "fractal.h"
...
public:
    ...
    static void compute_fractal(unsigned char* output, const fractal_params& params, 
                               int width, int height) {
        double dx = (double(params.x_max) - double(params.x_min)) / width;
        double dy = (double(params.y_max) - double(params.y_min)) / height;
        
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                double x = double(params.x_min) + col * dx;
                double y = double(params.y_min) + row * dy;
                ...
                if (params.fractal_type == 0) {
                    ...
                    // Julia set
                    iterations = julia_iterations(x, y, double(params.julia_cx), 
                                                double(params.julia_cy), params.max_iterations);
                }
...
int main(int argc, char* argv[]) {
    ...
    // Test configurations
    std::vector<fractal_params> test_configs = {
        // Mandelbrot set - classic view
        {-2.5, 1.0, -1.25, 1.25, 0.0, 0.0, 0, MAX_ITER},
        
        // Mandelbrot set - zoomed in
        {-0.8, -0.4, -0.2, 0.2, 0.0, 0.0, 0, MAX_ITER},
        
        // Julia set - classic spiral
        {-1.5, 1.5, -1.5, 1.5, -0.7, 0.27015, 1, MAX_ITER},
        
        // Julia set - dragon fractal
        {-1.5, 1.5, -1.5, 1.5, -0.8, 0.156, 1, MAX_ITER}
    };
    ...
    try {
        ...
        for (size_t config_idx = 0; config_idx < test_configs.size(); config_idx++) {
            ...
            std::cout << "X range: [" << float(params.x_min) << ", " << float(params.x_max) << "]" << std::endl;
            std::cout << "Y range: [" << float(params.y_min) << ", " << float(params.y_max) << "]" << std::endl;
            if (params.fractal_type == 1) {
                std::cout << "Julia constant: " << float(params.julia_cx) << " + " 
                          << float(params.julia_cy) << "i" << std::endl;
            }
