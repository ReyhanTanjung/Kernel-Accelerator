...
// // Matrix dimensions - keep at 32x32 to match FPGA hardware constraints
// #define M_SIZE 32
// #define K_SIZE 32 
// #define N_SIZE 32
...
//                 A_aligned[i * K_SIZE + j] = (i + j) * 0.1f;  // Match testbench pattern
//             }
//         }
        
//         for (int i = 0; i < K_SIZE; i++) {
//             for (int j = 0; j < N_SIZE; j++) {
//                 B_aligned[i * N_SIZE + j] = (i * j) * 0.01f;  // Match testbench pattern
//             }
//         }
        
//         for (int i = 0; i < M_SIZE; i++) {
//             for (int j = 0; j < N_SIZE; j++) {
//                 C_aligned[i * N_SIZE + j] = i - j;  // Match testbench pattern
//                 C_golden[i * N_SIZE + j] = C_aligned[i * N_SIZE + j];
//             }
//         }
        
//         // GEMM parameters - MATCH THOSE IN TEST BENCH
//         float alpha = 1.5f;  // Match alpha in testbench
//         float beta = 0.8f;   // Match beta in testbench
...
//                     sum += A_aligned[i * K_SIZE + l] * B_aligned[l * N_SIZE + j];
//                 }
//                 C_golden[i * N_SIZE + j] = alpha * sum + beta * C_golden[i * N_SIZE + j];
//             }
//         }
...
//         std::cout << "Executing GEMM kernel " << NUM_ITERATIONS << " times...\n";
...
//         // We can't verify against C_golden directly since we've done multiple iterations
//         // For simplicity, we'll just check if the values seem reasonable
//         std::cout << "Checking if results are reasonable...\n";
...
//         if (all_finite) {
//             std::cout << "All results are finite numbers. This is a basic sanity check.\n";
//             std::cout << "Note: Full verification is not performed for multiple iterations.\n";
//         } else {
//             std::cout << "Some results are not finite numbers! Something went wrong.\n";
//         }
...
//         std::cout << "\n" << std::string(40, '-') << "\n";
//         std::cout << "FPGA Performance metrics:\n";
